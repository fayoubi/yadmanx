...
import { ConversationStatus } from '../models/types.js';
...
export class LLMProvider {
     ...
     */
    async callClaudeWithRetry(params, sessionId) {
        let lastError;
     ...
     */
    buildSystemPrompt(extractedData, currentField) {
...
- Field names must be EXACT: gender, dateOfBirth, height, weight, city, usesNicotine, termLength`;
     ...
     */
    parseAIResponse(aiMessage, currentData) {
        try {
            ...
            if (!jsonString) {
                ...
                if (jsonMatch)
                    jsonString = jsonMatch[0].replace(/```/g, '');
            }
